...
void ThreadRateControl::doRateControl(int minLoopTimeMs) {
    ...
    if (remainingTime > 0) {
        ...
        HAL_UpdateNotifierAlarm((HAL_NotifierHandle)m_notifier, static_cast<uint64_t>(((frc::Timer::GetFPGATimestamp().value() + remainingTime) * 1e6) - 150), &m_notifierStatus);
...
double ThreadRateControl::getDt() {
    double currDtCalcTime = frc::Timer::GetFPGATimestamp().value();
